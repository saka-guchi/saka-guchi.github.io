...
- **設定UI（拡充）**:
  1.  **出題方法 (Method)**:
      - 選択肢: 「おまかせ (推奨)」「英→日」「英→日 (非表示)」「日→英」「例文: 英→日」「例文: 英→日 (非表示)」「例文: 日→英」「例文穴埋め」。
...
### 3.9 iOS ホーム画面追加バナー
- **表示条件**: iOS Safari/Chrome かつ PWA モードでない場合にホーム画面で表示。
- **内容**: 「共有ボタン → ホーム画面に追加」の手順を案内。
- **非表示条件**: 閉じるボタンで非表示後、`localStorage` に記録され再表示されない。

## 4. ロジック仕様

### 4.1 アダプティブクイズ (レベル別出題)
- 「おまかせ」モード時：
  - **Lv.0 (未学習)**: Standard (英→日)。
  - **Lv.1 (苦手)**: Masked (英→日 非表示)。
  - **Lv.2 (うろ覚え)**: Reverse (日→英)。
  - **Lv.3 (ほぼ覚えた)**: 例文ランダム (Ex-En-Ja または Ex-Ja-En)。
  - **Lv.4以上 (覚えた)**: 応用ランダム (Ex-En-Ja-Hidden または Fill-in)。
  - ※ 例文データがない単語は、Standard/Reverse 等に適宜フォールバックする。
...
- **メッセージ (Lv0-10)**:
  - 親密度レベル（0〜10）ごとに**各10種類**のメッセージを用意。
  - Lv0の「そっけない態度」から、Lv10の「魂のパートナー」まで段階的に変化する。
- **ポイント獲得**:
  - 正解: +1ポイント
...
- **通知モーダル**:
  - バージョン不一致時、更新内容（`RELEASE_NOTES`）をリスト表示。
  - **「最新版を適用」ボタン**: Service Workerのキャッシュを全削除し、ページを強制リロードして最新のアセットを適用する。
