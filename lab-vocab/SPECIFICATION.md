...
- **設定**:
  - モード（復習/新規）、出題数（10/20）、事前学習（ON/OFF）。
  - **対象レベル**: 自動(デフォルト), Lv.0〜Lv.5から選択可能。
    - 選択されたレベルの単語を優先的に出題。
    - 該当レベルの単語がない場合は、学習対象があればそれを使いつつ、**出題形式を選択レベルのものに強制**する。
- **デバッグ機能**: 削除。
- **バージョン情報**: フッター最下部に表示。
...
- **回答演出**:
  ...
  - **不正解時**: 選択したボタンを赤くすると同時に、**正解のボタンを緑色でハイライト**し、正答を視覚的に通知する。
- **機能**:
  - マスク機能（目のアイコン）。
  - 音声再生（スピーカーアイコン）。
...
- **レイアウト**:
  - ヘッダー: タイトルの横に今回のスコア（`正答数 / 出題数`）を表示。
  - 上部: 「記憶定着期間の変動」グラフを表示（前回からの変動を +/- で強調表示）。
  - 中央: 結果リスト（スクロール可能）。
    - 各アイテム表示:
      - 1行目: 英単語, 正誤マーク
      - 2行目: [品詞] 日本語訳
      - レベル変動: `Lv.Old -> Lv.New` 形式で表示。
